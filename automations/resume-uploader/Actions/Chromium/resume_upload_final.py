...
log_file = os.path.join(log_dir, todays_date+"_resume_uploader.log")
logging.basicConfig(
    filename=log_file,
    level=logging.INFO,
...
# Initialize WebDriver and Mimic Real User
options = Options()
...
try:
    ...
    email_field.send_keys("loginemail")
    ...
    # Allow time for login process (can be randomized between 5-10s for bot detection)
    time.sleep(5)
    ...
    resume_path = os.path.join(script_dir, "ResumeFile.pdf")
    upload_input = driver.find_element(By.ID, "attachCV")  # Locate the file input field and upload resume
    upload_input.send_keys(resume_path)

    # Allow some time for upload to complete (can be randomized between 5-10s for stronger non-bot detection)
    time.sleep(5)
